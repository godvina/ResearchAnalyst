...
class TextExtractor:
    ...
    def _try_extract(self, s3_key: str, batch_id: str) -> ExtractionResult:
        ...
        # For .txt files, read text directly (already extracted)
        if s3_key.lower().endswith(".txt"):
            try:
                text = file_bytes.decode("utf-8", errors="replace").strip()
                self._save_to_cache(s3_key, batch_id, text, "direct_text")
                return ExtractionResult(
                    s3_key=s3_key,
                    text=text,
                    method="direct_text",
                ...
                msg = f"Text file read failed: {exc}"
                logger.error("Failed to read text file %s: %s", s3_key, msg)
                return ExtractionResult(
                    s3_key=s3_key, text="", method="failed", char_count=0, error=msg
                )
        ...
        # For PDFs: Try PyPDF2
        try:
            text, page_count = self._extract_pypdf2(file_bytes)
        except Exception as exc:
